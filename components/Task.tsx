...
export default function Task({
	...
	nestedMaintenanceList: NestedMaintenance[];
}) {
	const { theme } = useTheme();
	const [mounted, setMounted] = useState(false);
	const [selectedTaskType, setSelectedTaskType] = useState("myTasks");
	...
	return (
		...
		>
			<Tabs
				...
				radius="md"
				color="primary"
				onSelectionChange={(key: Key) => setSelectedTaskType(key as string)}
			>
				<Tab key="myTasks" title="My Tasks" />
				<Tab key="completedTasks" title="Completed Tasks" />
				<Tab key="upcomingTasks" title="Upcoming Tasks" />
			</Tabs>
			{selectedTaskType === "myTasks" && (
				<div className="py-4">
					{nestedMaintenanceState.map((nestedMaintenance) => (
						<Card key={nestedMaintenance.uid} className="max-w-[400px] my-2">
			...
			{selectedTaskType === "completedTasks" && <p>Completed task here</p>}
			{selectedTaskType === "upcomingTasks" && <p>Upcoming task here</p>}
		</Card>
	);
}
